...
namespace Assets.MyPackman.Presenter
{
    ...
    {
        private readonly Vector3Int[] DirectionOffset = new Vector3Int[4] { new Vector3Int(-GameConstants.MovementStep, 0, 0),
                                                                            new Vector3Int(GameConstants.MovementStep, 0, 0),
                                                                            new Vector3Int(0, -GameConstants.MovementStep, 0),
                                                                            new Vector3Int(0, GameConstants.MovementStep, 0) };   // Вынести в настройки??

        private Coroutine _movement;
        private Transform _transform;
        private bool[] _directionsPresed = new bool[4];
        private int _lastDirection = GameConstants.NoDirection;
        ...
        public PlayerMovementHandler(Transform transform, IMapHandler handler, MonoBehaviour parent)
        {
            ...
            if (_mapHandler.TryFindPositionByObjectNumber(GameConstants.Player, ref _currentPosition) == false)
                throw new Exception(GameConstants.PositionOnMapNotFound);
        }
        ...
        public void RemoveMovementDirection(int removedDirection)
        {
            ...
            Moved -= Move;
            _lastDirection = GameConstants.NoDirection;
        }
        ...
        {
            int currentDirection = GameConstants.NoDirection;

            for (int i = 0; i < _directionsPresed.Length; i++)
                if (_directionsPresed[i])
                    currentDirection = i;

            if (_lastDirection != GameConstants.NoDirection)
                for (int i = 0; i < _directionsPresed.Length; i++)
        ...
        private IEnumerator Moving(Vector3Int nextPosition)
        {
            ...
            {
                _transform.position = Vector3.MoveTowards(_transform.position, correctPosition, GameConstants.PlayerSpeed);
                yield return null;
            }
            // По окончанию корутины сменить местоположение игрока на карте в MapHandler (через шину событий?)
            _mapHandler.ChangeTile(_currentPosition, GameConstants.EmptyTile);
            _mapHandler.ChangeTile(nextPosition, GameConstants.Player);
        ...
        private bool CanMove(Vector3Int position)
        {
            ...
            if (tile >= GameConstants.UpperLeftCornerWall && tile <= GameConstants.RightEndWall)
                return false;
