    ...
    public class LevelConstructor
    {
        ...
        private void ConstructLevel()
        {
            for (int y = 0; y < _map.Map.GetLength(0); y++)
            {
                ...
                {
                    if (_map.Map[y, x] > ConstantsGame.Player)
                        _tileMap.SetTile(new Vector3Int(x, -y), _textures[_map.Map[y, x]]);
                    else if (_map.Map[y, x] == ConstantsGame.Player)
                    {
                        var player = Object.Instantiate(_player);
                        player.transform.position = new Vector3(x * ConstantsGame.GridCellSize, -(y * ConstantsGame.GridCellSize));
